...
@Test func identityLifecycleIsOneActiveOpaqueAndSignatureBound() async throws {
    ...
    let request = try RadrootsOpaqueSignRequest(
        ...
        digest: digest,
        purpose: .nostrEvent,
        deadlineUnixMilliseconds: identityTestNow + 1000
    )
    ...
    let unlocked = try await fixture.custody.selectIdentity(
        identityHandle: #require(imported.identity?.identityHandle)
    )
...
private extension Data {
    func replacingOccurrences(of needle: Data, with replacement: Data) -> Data {
