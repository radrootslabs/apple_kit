...
@Test func backgroundTransferRequestValidatesUrlMethodAndHeaders() throws {
    ...
    let request = try RadrootsBackgroundTransferRequest(
        ...
        remoteURL: URL(string: "https://radroots.org/relay.json")!, method: .get, operation: .download(destination: destination),
    ...
    let longAuthorization = String(repeating: "a", count: 2_048)
    ...
    #expect(throws: RadrootsBackgroundTransferError.invalidRequest("background transfer remote URL must use public HTTPS")) {
        _ = try RadrootsBackgroundTransferRequest(
            remoteURL: URL(string: "http://radroots.org/relay.json")!, method: .get, operation: .download(destination: destination))
    }
    #expect(throws: RadrootsBackgroundTransferError.invalidRequest("background transfer remote URL is unsafe")) {
        _ = try RadrootsBackgroundTransferRequest(
            remoteURL: URL(string: "https://radroots.org/relay.json?token=secret")!, method: .get,
            operation: .download(destination: destination))
    }
    let simulatorRequest = try RadrootsBackgroundTransferRequest(
        remoteURL: URL(string: "http://127.0.0.1:21100/relay.json")!, method: .get, operation: .download(destination: destination),
    ...
    #expect(throws: RadrootsBackgroundTransferError.invalidRequest("background download transfers must use GET")) {
        _ = try RadrootsBackgroundTransferRequest(
            remoteURL: URL(string: "https://radroots.org/relay.json")!, method: .post, operation: .download(destination: destination))
    }
...
@Test func backgroundTransferUploadRequiresUploadMethod() throws {
    let source = RadrootsBackgroundTransferLocalFile.stagedBlob(try RadrootsStagedBlobReference(blobID: "upload", sizeBytes: 12))

    let request = try RadrootsBackgroundTransferRequest(
        remoteURL: URL(string: "https://radroots.org/upload")!, method: .put, operation: .upload(source: source))
    ...
    #expect(throws: RadrootsBackgroundTransferError.invalidRequest("background upload transfers must use POST or PUT")) {
        _ = try RadrootsBackgroundTransferRequest(
            remoteURL: URL(string: "https://radroots.org/upload")!, method: .get, operation: .upload(source: source))
    }
}
...
@Test func backgroundTransferPersistenceRedactsSecretsAndValidatesDecode() async throws {
    ...
    let request = try RadrootsBackgroundTransferRequest(
        identifier: RadrootsBackgroundTransferIdentifier("field.transfer.redacted"), remoteURL: URL(string: "https://radroots.org/upload")!,
    ...
    try await store.saveSnapshot(try RadrootsBackgroundTransferSnapshot(request: request))
...
@Test func appleBackgroundTransferStorePersistsBoundedResponseAcrossRestart() async throws {
    ...
    let request = try RadrootsBackgroundTransferRequest(
        ...
        remoteURL: URL(string: "https://blossom.radroots.org/upload")!, method: .put,
    ...
    try await store.saveSnapshot(try RadrootsBackgroundTransferSnapshot(request: request, state: .completed, response: response))
