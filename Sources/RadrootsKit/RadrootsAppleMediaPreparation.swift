...
extension RadrootsAppleMediaPreparationError: LocalizedError {
    public var errorDescription: String? {
        switch self {
        case .invalidRequest(let message), .unavailable(let message), .preparationFailure(let message): message
        }
    }
}
...
public struct RadrootsAppleImagePreparationRequest: Sendable, Equatable, Hashable {
    ...
    public init(
        source: RadrootsBackgroundTransferLocalFile, maximumInputBytes: Int = 40 * 1_024 * 1_024,
        maximumOutputBytes: Int = 10 * 1_024 * 1_024, maximumPixelCount: Int = 40_000_000, maximumDimension: Int = 4_096
    ) throws {
        guard (1...(40 * 1_024 * 1_024)).contains(maximumInputBytes), (1...(10 * 1_024 * 1_024)).contains(maximumOutputBytes),
            (1...40_000_000).contains(maximumPixelCount), (1...8_192).contains(maximumDimension)
...
public actor RadrootsAppleMediaPreparer {
    ...
    public init(
        ...
        self.roots = roots
        self.resolver = RadrootsAppleBackgroundTransferFileResolver(roots: roots)
    ...
    private func prepareValidatedImage(_ request: RadrootsAppleImagePreparationRequest) async throws -> RadrootsApplePreparedImage {
        ...
        defer { if fileManager.fileExists(atPath: temporaryURL.path) { try? fileManager.removeItem(at: temporaryURL) } }
